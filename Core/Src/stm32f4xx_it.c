...
void USART2_IRQHandler(void)
{
  ...
  /* USER CODE BEGIN USART2_IRQn 1 */
    ...
    if(USART2 == huart2.Instance)//判断是否为串口2中断  USART6
    {
        if(RESET != __HAL_UART_GET_FLAG(&huart2,UART_FLAG_IDLE))//如果为串口2空闲
        {
            ...
            temp  = __HAL_DMA_GET_COUNTER(&hdma_usart2_rx);//获取DMA当前还有多少未填充 hdma_usart6_rx
            Rx_len_Huart2 =  BUFFERSIZE - temp; //计算串口接收到的数据个数 Rx_len_Huart6
            /*************************************************************************/
            //接收数据处理
            if(Rx_len_Huart2==28)
            {
